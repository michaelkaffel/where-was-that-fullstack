...
## Decisions Log
- **2026-09-14 — Restricted `GET /users/:userId` to admins.** Docs review of `/docs` against the actual route handlers turned up an undocumented route with no ownership or admin check (only `verifyUser`): any authenticated user could fetch any other user's full profile (email, `admin` flag, `googleId`, `hasPassword`) by guessing/enumerating an id. No client code called it. Fixed by adding `verifyAdmin`, matching `DELETE /users/:userId`. Same pass also found `PATCH /users/me`'s actual body shape (`currentPassword`/`newPassword`, plus `firstname`/`lastname` in `allowedFields`) didn't match `api-contract.json`, and that signup uses a plain `User.register()` call with no stream workaround (only login bypasses `passport.authenticate('local')`) — both corrected across the JSON docs.
